...
export type PurifyCommandOutputAsync<
  B = any,
  Es extends Array<Event> = []
> = EitherAsync<Error, PurifyCommandOutput<B, Es>>;

export interface PurifyCommandHandler<
...
export function createPurifyCommandOutput<
  B = any,
  Es extends Array<Event> = []
>(data: Partial<PurifyCommandOutput<B, Es>> = {}): PurifyCommandOutput<B, Es> {
  return {
    result: Maybe.zero(),
    events: Maybe.zero(),
    ...data
...
export class PurifyCommandBus {
  ...
  handle<
    ...
    B = any,
    Es extends Array<Event> = [],
    R extends Result<B> = Result
  >(commandType: string, handler: PurifyCommandHandler<C, B, Es>): Removable {
